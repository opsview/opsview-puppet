...
Puppet::Type.type(:opsview_bsmcomponent).provide :opsview, :parent => Puppet::Provider::Opsview do
  ...
  def self.object_map(object)
    ...
    p[:required_online] = (object["hosts"].count * ( object["quorum_pct"].to_f/100.0) ).round().to_s if defined?object["quorum_pct"]
  ...
  def flush
    ...
    if not @property_hash[:required_online].to_s.empty?
      ...
      @upload_json["quorum_pct"] = (calculated_pct % 1 == 0.00) ? calculated_pct.truncate : calculated_pct
    end
  ...
  def default_object
   json = '
   {
         "name" : "Puppet-created bsm component",
         "has_icon" : "0",
         "host_template" : {
            "name" : "Network - Base"
         },
