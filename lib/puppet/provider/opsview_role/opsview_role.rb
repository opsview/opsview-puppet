...
Puppet::Type.type(:opsview_role).provide :opsview, :parent => Puppet::Provider::Opsview do
  ...
  def self.generic_fields
    [:all_bsm_components, :all_bsm_edit, :all_bsm_view, :all_hostgroups, :all_hashtags, :all_monitoringservers, :all_servicegroups]
  end
  ...
  def self.generic_array_name_fields
    [:configure_hostgroups, :hostgroups, :hashtags, :servicegroups, :access, :monitoring_servers]
  end
  ...
  def self.puppet_map
  {
    ...
    :configure_hostgroups => "hostgroups",
    :hashtags => "access_keywords",
    :hostgroups => "access_hostgroups",
    :monitoring_servers => "monitoringservers",
    :servicegroups => "access_servicegroups"
  } 
  end
  ...
  def self.object_map(object)
    ...
    if defined?object["business_services"] and object["business_services"].is_a?(Array)
      truth_map={ "0" => "false", "1" => "true"}
      p[:business_services] = object["business_services"].collect{ |c|
        {"name" => c["name"], "edit" => truth_map[c["edit"]]}.delete_if{ |k, v| v.nil?}
       }
    end
  ...
  def flush
    ...
    if @property_hash[:business_services] and @property_hash[:business_services].is_a?(Array)
      truth_map={ "false" => "0", "true" => "1"}
      @upload_json["business_services"] = @property_hash[:business_services].collect{ |c|
        {:name => c["name"], :edit => truth_map[c["edit"]]}
      }
    end
