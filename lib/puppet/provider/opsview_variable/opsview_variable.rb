...
Puppet::Type.type(:opsview_variable).provide :opsview, :parent => Puppet::Provider::Opsview do
  ...
  def self.generic_fields
    [:name, :secured1, :secured2, :secured3, :secured4]
  end

  def self.generic_blankable_fields
    [:arg1, :arg2, :arg3, :arg4, :encrypted_arg1, :encrypted_arg2, :encrypted_arg3, :encrypted_arg4, :label1, :label2, :label3, :label4, :value]
  end
  ...
  def flush
    ...
    #Do some cleanup if we are moving from encrypted to unencrypted or vice-versa
    [1,2,3,4].each do |num|
      argument="arg"+num.to_s
      encrypted_argument="encrypted_"+argument
      secured="secured"+num.to_s
      if not @resource[encrypted_argument].nil?
        @upload_json.delete(argument) if defined?@upload_json[argument]
	@upload_json[secured] = "1"
      elsif not @resource[argument.to_sym].nil?
        @upload_json.delete(encrypted_argument) if defined?@upload_json[encrypted_argument]
	@upload_json[secured] = "0"
      end
    end
